...
export async function getClientAppointments(): Promise<Appointment[]> {
  ...
  if (!res.ok)
    throw new Error(
      (await res.json()).message || "Failed to fetch client appointments",
    );
...
export async function createAppointment(
  ...
  if (!res.ok)
    throw new Error(
      (await res.json()).message || "Failed to create appointment",
    );
  return res.json();
}
...
export async function rescheduleAppointment(
  ...
  if (!res.ok)
    throw new Error(
      (await res.json()).message || "Failed to reschedule appointment",
    );
  return res.json();
}
...
export const deleteAppointment = async (id: string) => {
  ...
  if (!res.ok)
    throw new Error(
      (await res.json()).message || "Failed to delete temp appointment",
    );
  return res.json();
};
